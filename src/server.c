...
static const char *zenerr(int rc) {
    switch (rc) {
        case -ERRCLIENTDC:
...
static void tcp_session_init(struct tcp_session *tcp) {
    ...
    if (!tcp->stream.buf)
        tcp->stream.buf =
            zen_calloc(MAX_STREAM_BUF_SIZE, sizeof(unsigned char));
}
...
static inline int tcp_session_read(struct tcp_session *tcp) {
    ...
    if (conf->mode == ZEN_TCP_MODE && nread == 0)
        return -ERRCLIENTDC;

    tcp->stream.toread =
        tcp->stream.toread == 0 ? 0 : tcp->stream.toread - nread;

    if ((conf->mode == ZEN_HTTP_MODE || tcp->stream.toread > 0)
    ...
    switch (rc) {
        ...
        case -ERRSOCKETERR:
            ...
            log_error("Closing connection with %s -> %s: %s",
                      ...
                      http->tcp_session.pipe[BACKEND].ip, zenerr(rc));
            http_transaction_close(http);
            break;
        case -ERREAGAIN:
 ...
 * the balancing algorithm available, currently zen supports the following
 * balancing algorithm:
 *
 * - Round robin
 * - Random
...
int start_server(const struct frontend *frontends, int frontends_nr) {
    ...
    struct listen_payload loop_start = {
        .fds = zen_calloc(frontends_nr, sizeof(struct frontend)),
    ...
    if (conf->mode == ZEN_HTTP_MODE) {
        ...
        for (size_t i = 0; i < MAX_ACTIVE_SESSIONS; ++i) {
            ...
            if (ptr->tcp_session.stream.buf)
                zen_free(ptr->tcp_session.stream.buf);
        }
    } else {
        ...
        for (size_t i = 0; i < MAX_ACTIVE_SESSIONS; ++i) {
            ptr = memorypool_advance_pointer(server.pool, i);
            if (ptr->stream.buf)
                zen_free(ptr->stream.buf);
        }
    }

    memorypool_destroy(server.pool);
    zen_free(loop_start.fds);

    log_info("zen v%s exiting", VERSION);
