...
static void ev_api_init(struct ev_ctx *ctx, int events_nr) {
    struct epoll_api *e_api = zen_malloc(sizeof(*e_api));
    e_api->fd = epoll_create1(0);
    e_api->events = zen_calloc(events_nr, sizeof(struct epoll_event));
    ctx->api = e_api;
    ctx->maxfd = events_nr;
}
...
static void ev_api_destroy(struct ev_ctx *ctx) {
    ...
    zen_free(((struct epoll_api *) ctx->api)->events);
    zen_free(ctx->api);
}
...
static void ev_api_init(struct ev_ctx *ctx, int events_nr) {
    struct poll_api *p_api = zen_malloc(sizeof(*p_api));
    p_api->nfds = 0;
    p_api->fds = zen_calloc(events_nr, sizeof(struct pollfd));
...
static void ev_api_destroy(struct ev_ctx *ctx) {
    zen_free(((struct poll_api *) ctx->api)->fds);
    zen_free(ctx->api);
}
...
static int ev_api_watch_fd(struct ev_ctx *ctx, int fd) {
    ...
    if (p_api->nfds >= p_api->events_monitored) {
        ...
        p_api->fds = zen_realloc(p_api->fds,
...
static void ev_api_init(struct ev_ctx *ctx, int events_nr) {
    ...
    struct select_api *s_api = zen_malloc(sizeof(*s_api));
    FD_ZERO(&s_api->rfds);
    FD_ZERO(&s_api->wfds);
    ctx->api = s_api;
    ctx->maxfd = 0;
...
static void ev_api_init(struct ev_ctx *ctx, int events_nr) {
    struct kqueue_api *k_api = zen_malloc(sizeof(*k_api));
    k_api->fd = kqueue();
    k_api->events = zen_calloc(events_nr, sizeof(struct kevent));
    ctx->api = k_api;
    ctx->maxfd = events_nr;
}
...
static void ev_api_destroy(struct ev_ctx *ctx) {
    ...
    zen_free(((struct kqueue_api *) ctx->api)->events);
    zen_free(ctx->api);
}
    ...
    if (fd > ctx->maxevents) {
        ...
        if (fd > ctx->events_nr) {
            ctx->events_monitored =
                zen_realloc(ctx->events_monitored, (fd + 1) * sizeof(struct ev));
...
void ev_init(struct ev_ctx *ctx, int events_nr) {
    ...
    ctx->events_monitored = zen_calloc(events_nr, sizeof(struct ev));
}
