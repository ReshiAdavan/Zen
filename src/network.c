...
ssize_t stream_recv(int fd, struct stream *stream) {
    ...
    do {
        ...
        if (stream->size == stream->capacity) {
            stream->capacity *= 2;
            stream->buf = zen_realloc(stream->buf, stream->capacity);
        }
    } while (n > 0);
...
SSL_CTX *create_ssl_context() {
    ...
    if (!(conf->tls_protocols & ZEN_TLSv1))
        SSL_CTX_set_options(ctx, SSL_OP_NO_TLSv1);
    if (!(conf->tls_protocols & ZEN_TLSv1_1))
        SSL_CTX_set_options(ctx, SSL_OP_NO_TLSv1_1);
#ifdef SSL_OP_NO_TLSv1_2
    if (!(conf->tls_protocols & ZEN_TLSv1_2))
        SSL_CTX_set_options(ctx, SSL_OP_NO_TLSv1_2);
#endif
#ifdef SSL_OP_NO_TLSv1_3
    if (!(conf->tls_protocols & ZEN_TLSv1_3))
    ...
    while (stream->size > 0) {
        if ((n = SSL_write(ssl, stream->buf + n, stream->size)) <= 0) {
            ...
            if (err == SSL_ERROR_ZERO_RETURN
                ...
                return ZEN_SUCCESS;  // Connection closed
            if (errno == EAGAIN || errno == EWOULDBLOCK)
                break;
            else
                goto err;
...
struct connection *connection_new(const SSL_CTX *ssl_ctx) {
    struct connection *conn = zen_malloc(sizeof(*conn));
    if (!conn)
        return NULL;
