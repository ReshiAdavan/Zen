...
static void add_config_value(const char *key, const char *value) {
    ...
    if (STREQ("log_level", key, klen) == true) {
        ...
        if (STREQ("http", value, 4))
            config.mode = ZEN_HTTP_MODE;
        else if (STREQ("tcp", value, 3))
            config.mode = ZEN_TCP_MODE;
    }
}
...
bool config_load(const char *configpath) {
    ...
    if (!fh) {
        ...
        log_warning("To specify a config file run zen -c /path/to/conf");
        return false;
    }
...
void config_set_default(void) {
    ...
    config.max_backends_nr = 2;
    config.frontends = zen_calloc(config.max_frontends_nr,
                                  sizeof(struct frontend));
    config.backends = zen_calloc(config.max_backends_nr,
...
void config_print_tls_versions(void) {
    char protocols[64] = {0};
    int pos = 0;
    if (config.tls_protocols & ZEN_TLSv1) {
        strncpy(protocols, "TLSv1, ", 64);
        pos += 7;
    }
    if (config.tls_protocols & ZEN_TLSv1_1) {
        strncpy(protocols + pos, "TLSv1_1, ", 64 - pos);
        pos += 9;
    }
    if (config.tls_protocols & ZEN_TLSv1_2) {
        strncpy(protocols + pos, "TLSv1_2, ", 64 - pos);
        pos += 9;
    }
    if (config.tls_protocols & ZEN_TLSv1_3) {
...
void config_print(void) {
    ...
    log_info("zen v%s is starting", VERSION);
    log_info("Mode: %s", conf->mode == ZEN_HTTP_MODE ? "http" : "tcp");
...
void config_unload(void) {
    zen_free(config.backends);
    zen_free(config.frontends);
}
